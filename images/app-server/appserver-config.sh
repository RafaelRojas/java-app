...
## Below lines may have to change if Oracle Java has to be used instead of openjdk
apt-get install -y \
  openjdk-8-jdk \
  openjdk-8-jre \
...
# Install Spring
## Installing Spring through script sucks :( this may be handled better using either Ansible or Chef
...
tar -xzf spring-boot-cli-2.1.4.RELEASE-bin.tar.gz -C /opt/spring-boot
